...
/**
 * <h1>AmazonViewer</h1>
 * AmazonViewer es un programa del curso de platzi java avanzado
 * <p>
 * Tiene reglas como que los elementos pueden ser visualizados, menos las revistas
 *  las cuales s[olo se muestran sin ser leidas.
 *  </p>
 *  @author Aldo Valdez
 *  @version 1.0
 *  @since 2021
...
//		ArrayList<Movie> moviesD = Movie.getMoviesDate(dateF);
		//Peliculas
		Movie.getMoviesDate(dateF).stream().forEach(m-> contentReport.append(m.toString()+"\n"));
		//series
		Consumer<Serie> seriesEach = s -> {
			ArrayList<Chapter> chapters = s.getChapters();
			chapters.stream().filter(c->c.getIsViewed()).forEach(c->contentReport.append(c.toString() +"\n"));			
